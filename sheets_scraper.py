...
Checks if utilities are using satellite data (e.g., Overstory) for inspections
"""
...
class SheetsSatelliteChecker:
    """Checks utilities in Google Sheets for satellite data usage"""
    ...
    def find_start_row(self, column_index: int) -> int:
        ...
        For "Using Satellite", looks for empty cells (no True/False)
        For "Source", looks for empty cells (no URLs)
    ...
    def check_satellite_usage(self, utility_name: str) -> Tuple[bool, List[str]]:
        """
        Check if utility uses satellite data for inspections
        
        Requires: utility name (any variation) AND ("satellite" OR "overstory" OR "AiDash") 
                  AND ("vegetation management" OR related terms)
        
        Args:
            utility_name: Name of the utility
        
        Returns:
            Tuple of (is_using_satellite: bool, source_urls: List[str])
        """
        ...
        # Format: (utility1 OR utility2 OR ...) AND ("satellite" OR "overstory" OR "AiDash" OR "ai") 
        #         AND ("vegetation" OR "right-of-way" OR "ROW" OR "right of way")
        # Using quotes around keywords to ensure exact matches
        # Note: Google Search is case-insensitive, but we also validate case-insensitively in code
        utility_terms = " OR ".join([f'"{var}"' for var in utility_variations])
        satellite_terms = '"satellite" OR "overstory" OR "AiDash" OR "ai"'
        vegetation_terms = '"vegetation" OR "right-of-way" OR "ROW" OR "right of way"'
        query = f'({utility_terms}) ({satellite_terms}) AND ({vegetation_terms})'
        
        print(f"  Checking: {utility_name}...")
        source_urls = []
        
        # Keywords that must appear (case-insensitive - all checked as lowercase)
        # At least one satellite keyword required
        required_satellite_keywords = ['satellite', 'overstory', 'aidash', 'ai']
        # At least one vegetation keyword required
        required_vegetation_keywords = ['vegetation', 'right-of-way', 'row', 'right of way']
        ...
        try:
            ...
            # Check each result - must contain utility name (any variation) AND one of the keywords
            # 100% validation: both must appear in page content
            for result in organic_results:
                ...
                # Exclude pages with "satellite dish" or related phrases
                banned_satellite_phrases = ['satellite dish', 'satellite dishes', 'dish satellite', 'tv satellite dish', 'satellite internet', 'satellite campus', 'satellite facility', 'overstory trees', 'satellite box', 'satellite boxes']
                if any(phrase in page_content_lower for phrase in banned_satellite_phrases):
                    continue
                
                # Exclude pages containing "cable", "broadband", "campus", "iecl", "issuu", or "indianaconnection"
                if 'cable' in page_content_lower or 'broadband' in page_content_lower or 'campus' in page_content_lower or 'iecl' in page_content_lower or 'issuu' in page_content_lower or 'indianaconnection' in page_content_lower:
                    continue
                
                # Check if utility name (any variation) appears (case-insensitive)
                utility_found = any(var_lower in page_content_lower for var_lower in utility_variations_lower)
                
                # Check if at least one satellite keyword appears (case-insensitive)
                # Must appear in title or snippet - 100% validation
                satellite_keyword_found = False
                for keyword in required_satellite_keywords:
                    if keyword in page_content_lower:
                        satellite_keyword_found = True
                        break
                
                # Check if at least one vegetation keyword appears (case-insensitive)
                # Must appear in title or snippet - 100% validation
                vegetation_keyword_found = False
                for keyword in required_vegetation_keywords:
                    if keyword in page_content_lower:
                        vegetation_keyword_found = True
                        break
                
                # All three must be present - 100% validation ensures keywords are in the page content
                # Utility name AND satellite keyword AND vegetation keyword
                if utility_found and satellite_keyword_found and vegetation_keyword_found:
            ...
            if found_evidence:
                print(f"    ✓ Found {len(source_urls)} source URL(s) with satellite inspection information")
            else:
                print(f"    ✗ No evidence of satellite usage found")
    ...
    def update_sheet(
        ...
        Args:
            results: List of tuples (row_number, is_using_satellite, source_urls)
            satellite_col_index: Column index for "Using Satellite" (1-based)
            source_col_index: Column index for "Source" (1-based)
        """
        if not results:
        ...
        try:
            ...
            satellite_col = col_index_to_letter(satellite_col_index)
            source_col = col_index_to_letter(source_col_index)
            
            # Prepare batch update data
            # Since rows may not be consecutive, we need to map each row number to its value
            batch_updates = []
            
            for row_num, is_using_satellite, source_urls in results:
                # Create individual cell updates for each row
                # This ensures we update the correct rows even if they're not consecutive
                satellite_range = f"{satellite_col}{row_num}"
                batch_updates.append({
                    'range': satellite_range,
                    'values': [[str(is_using_satellite)]]
                })
            ...
            if batch_updates:
                ...
                # Even with individual cell ranges, this is much more efficient than update_cell()
            ...
            for row_num, is_using_satellite, source_urls in results:
                try:
                    self.worksheet.update_cell(row_num, satellite_col_index, str(is_using_satellite))
                    source_text = ", ".join(source_urls) if source_urls else ""
    ...
    def run(self, gid: Optional[str] = None, limit: Optional[int] = None):
        """
        Run the satellite usage check for utilities
        ...
        satellite_col_index = self.find_or_create_column("Using Satellite")
        source_col_index = self.find_or_create_column("Source")
        
        # Find the starting row based on "Using Satellite" column (primary)
        # This ensures we continue from where we left off
        start_row = self.find_start_row(satellite_col_index)
        
        print(f"✓ Starting to populate from row {start_row} (first empty row in 'Using Satellite' column)")
        if limit:
        ...
        print(f"\nChecking satellite usage for {len(utilities)} utilities...")
        results = []
        
        for row_num, utility_name in utilities:
            try:
                is_using_satellite, source_urls = self.check_satellite_usage(utility_name)
                results.append((row_num, is_using_satellite, source_urls))
        ...
        self.update_sheet(results, satellite_col_index, source_col_index)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check utilities in Google Sheets for satellite data usage"
    )
    parser.add_argument(
        "--sheet-id",
    ...
    try:
        checker = SheetsSatelliteChecker(
